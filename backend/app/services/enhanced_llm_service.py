...
class EnhancedLLMService:
    """
    Enhanced LLM Service with improved prompt engineering, few-shot learning,
    ...
    def _get_enhanced_system_prompt(self, user_context: Optional[Dict[str, Any]] = None) -> str:
...
Tailor your responses to this student's specific context and developmental stage."""
    ...
    async def analyze_resume_enhanced(self, resume_text: str, user_context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Enhanced resume analysis with few-shot learning and chain-of-thought reasoning"""
...
Apply the reasoning process step-by-step, then provide the complete JSON analysis following the schema above. Focus on understanding this person's unique journey and how their experiences reflect Ignatian values of service, growth, and authentic development."""
        ...
        try:
            ...
            return self._parse_json_response_enhanced(response)
        except Exception as e:
            return self._create_error_response("resume analysis", str(e))

    async def analyze_job_description_enhanced(self, job_text: str, user_context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Enhanced job description analysis with cultural values extraction"""
...
Follow the step-by-step reasoning process, then provide the complete JSON analysis. Focus on understanding not just what skills they want, but what kind of person and values they're seeking, and how this role could serve the common good."""
    ...
    async def find_connections_enhanced(self, resume_analysis: Dict[str, Any], job_analysis: Dict[str, Any], user_context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
...
Provide a rich, nuanced synthesis that helps this student understand the deeper patterns and meaning in their journey. Focus on authentic discovery rather than what sounds good, and help them see how their experiences point toward their unique calling and contribution to the world."""
    ...
    async def generate_ignatian_reflection_prompts_enhanced(self, synthesis_analysis: Dict[str, Any], user_context: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
...
Generate 8 total prompts (2 per category) that are deeply personalized to this student's unique journey, values, and calling indicators. Each prompt should invite them into genuine contemplation and help them discern their authentic path forward."""
        ...
        try:
            response = await self._call_openai_enhanced(prompt, user_context)
            result = self._parse_json_response_enhanced(response)
            return result.get('prompts', [])
        except Exception as e:
            return self._create_fallback_reflection_prompts()

    async def generate_portfolio_project_enhanced(self, synthesis_analysis: Dict[str, Any], reflection_responses: Dict[str, str], user_context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
...
Design a portfolio project that feels like an authentic expression of this student's unique gifts, values, and calling while demonstrating professional competence and creating real value in the world. The project should feel both challenging and energizing, stretching their capabilities while aligning with their deeper purpose."""
    ...
    async def generate_interview_questions_enhanced(self, project_plan: Dict[str, Any], connections_analysis: Dict[str, Any], user_context: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
...
Generate 8-10 interview questions across all categories that would be realistic for their target role. Focus on questions that allow them to showcase both their technical competence and their values-driven approach to work. Provide detailed, practical guidance that helps them prepare authentic, compelling responses."""
    ...
    async def _call_openai_enhanced(self, prompt: str, user_context: Optional[Dict[str, Any]] = None) -> str:
        """Enhanced OpenAI API call with user context and improved error handling"""
        try:
            system_prompt = self._get_enhanced_system_prompt(user_context)
            ...
            raise Exception(f"Enhanced OpenAI API call failed: {str(e)}")
